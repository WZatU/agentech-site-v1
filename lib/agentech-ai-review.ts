...
const softwareSecurityReviewPrompt = [
  ...
  "- Public Agentech L0.5 robot API calls: forward, backward, lateral, diagonal, turn, yaw, pitch, roll, stay, backflip, jump, stand, squat, squat_forward, squat_backward, squat_lateral, squat_diagonal, squat_turn, sit, stop, emergency_stop, get_battery_status, get_body_state, and capture_image.",
  "- Supported L0.5 named profiles use the documented SDK parameters, including speed_mps/duration_s, speed_percent, speed_level, distance_m/duration_s, distance_m/speed_mps, coordinate or angle-based diagonals, signed turn angles/rates, radian posture targets, wait, output, and source.",
  "- Calls such as Agentech.forward(speed_percent=40, duration_s=1.0), Agentech.lateral_left(speed_mps=0.5, duration_s=2.0), Agentech.diagonal(x_m=0.5, y_m=1.0, duration_s=2.0), Agentech.squat_forward(speed_mps=0.5, duration_s=2.0), Agentech.squat_backward(speed_mps=0.5, duration_s=2.0), Agentech.squat_lateral(direction=\"left\", speed_mps=0.5, duration_s=2.0), Agentech.squat_diagonal(angle_deg=45, speed_mps=0.5, duration_s=2.0), Agentech.squat_turn(angle_deg=90), Agentech.turn(angle_deg=45, turn_rate_deg_s=22.5), Agentech.stay(duration_s=1.0), Agentech.sit(), Agentech.stop(), Agentech.emergency_stop(), Agentech.get_battery_status(), Agentech.get_body_state(), and Agentech.capture_image(output=\"capture.jpg\", source=\"default\").",
