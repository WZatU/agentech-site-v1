...
Show Master's four primary RGB cameras in the website's existing Live Stream area during an active scheduled Master session. The free viewing experience must never exceed one 4K output: users may watch a 2x2 wall made from four 1080p views or focus on one camera at up to its native 4K resolution.
...
| Rear View | `rear-view` | `/aima/hal/sensor/rgb_head_rear/rgb_image/compressed` |
...
- **Camera Wall:** a single 3840x2160 output arranged as a 2x2 grid. Each quadrant is allocated 1920x1080 and labeled with its camera name.
- **Focus View:** a single selected RGB camera fills the 3840x2160 output. The user may select Front Main, Front Left, Front Right, or Rear View. The other three camera subscriptions stop while focus mode is active.
...
A Master compositor module on that gateway consumes the four approved RGB feeds through the local Master Robot Vision relay. It produces exactly one video program:

- wall mode composites the latest four frames into one 3840x2160 2x2 canvas;
...
- mode: `wall` or `focus`;
- camera ID: one of the four allowlisted RGB sensor IDs when mode is `focus`.
...
- focus mode allows one camera at up to 3840x2160 and disables the other three subscriptions;
...
- the four-camera allowlist and exclusion of RGB-D, depth, and LiDAR;
- 2x2 placement and 1080p quadrant allocation in a 4K canvas;
