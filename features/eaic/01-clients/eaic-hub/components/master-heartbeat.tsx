...
export function MasterHeartbeat() {
  ...
  return (
    ...
    >
      <div className="mb-2 flex items-center gap-2 text-sm font-bold text-[#173b62]">
      ...
      <dl className="grid gap-2 text-xs sm:grid-cols-5">
        {fields.map(([label, value]) => (
          <div key={label} className="border border-black/10 bg-white/70 px-2 py-1.5" title={label === "Last update" ? status?.receivedAt || undefined : undefined}>
            <dt className="font-semibold text-[#52677d]">{label}</dt>
            <dd className="mt-0.5 text-[#173b62]">{value}</dd>
          </div>
        ))}
      </dl>
    </section>
