"""Render Navi's front-stretch choreography with planted contact targets."""
...
def phases(t: float) -> tuple[float, float]:
    # Only the forebody lowers. The rear paws and normal rear-leg stance stay
    # fixed while the rear hips remain visibly raised.
    elbow_in = ramp(t, 1.20, 2.15)
    recovery = ramp(t, 4.15, 5.25)
    elbow = elbow_in * (1.0 - recovery)
    return elbow, 0.0


def pose(t: float) -> tuple[float, float, float, float, float]:
    elbow, _ = phases(t)
    shake = 0.0
    if 0.18 <= t <= 1.02:
        envelope = math.sin(math.pi * (t - 0.18) / 0.84) ** 2
        shake = math.sin(2.0 * math.pi * 2.2 * (t - 0.18)) * envelope
    elif 2.85 <= t <= 3.95:
        envelope = math.sin(math.pi * (t - 2.85) / 1.10) ** 2
        shake = math.sin(2.0 * math.pi * 2.0 * (t - 2.85)) * envelope

    # Pitching the chest down keeps the rear hips high and the rear legs in
    # their regular stance while both front elbows reach the floor.
    roll = math.radians(3.5) * shake
    pitch = math.radians(15.0) * elbow
    z = -0.050 * elbow
    x = -0.100 * elbow
    y = 0.010 * shake
    return roll, pitch, z, x, y


def foot_target(t: float, leg: str, standing_target: np.ndarray) -> np.ndarray:
    elbow, _ = phases(t)
    target = standing_target.copy()
    if leg.startswith("front"):
        target[0] += 0.040 * elbow
    return target
...
def main() -> int:
    ...
    metrics = {
        "command": "front_stretch",
        "duration_s": DURATION_S,
        "visible_sequence": "shake_then_front_elbows_ground_with_rear_hips_up_then_shake_then_stand",
